        ...
        if results.multi_hand_landmarks:
            for hand_landmark in results.multi_hand_landmarks:
                ...
                for id, lm in enumerate(
                        hand_landmark.landmark):  # ennumarate voegt nummers toe aan een lijst van objecten
                    lm_list.append(lm)  # append is toevoegen van een item aan de lijst.
                finger_fold_status = []

                for tip in fingerTips:
                    x, y = int(lm_list[tip].x * w), int(
                        lm_list[tip].y * h)  # alle vingertoppen uit de lijst x maal breedte, y maal hoogte
                    # print(id, ":", x, y) #geeft nu positie breedte en hoogte landmark in int ipv float
                    cv2.circle(img, (x, y), 10, (0, 255, 0),
                               cv2.FILLED)  # hiermee teken ik een dikkere cirkel om een specifiek punt
                    ...
                    mp_draw.draw_landmarks(img, hand_landmark, mp_hands.HAND_CONNECTIONS,
                                           ...
                                           mp_draw.DrawingSpec((153, 51, 255), 2, 2)
                                           # drawing specs via Tulp kleuren controle
                                           )
