

class Message:
    def __int__(self):
        console_message_explain()


def console_message_explain():
    print("Welkom bij het spel: RAAD DE CODE!! ")
    print("Doel van het spel: raad de code door je vingers op te steken")
    print("=======================")
    print("\t1. Steek je vingers op om nummers te maken")
    print("\t2. Druk op de knop V om een poging te wagen")
    print("\t3. Heb je het goed? Dan komt hij bij de code te staan!")
    print("\t4. Heb je het fout? Dan verschijnt er een foutmelding!")
    print("\n=======================")
    print("\nHEEL VEEL SUCCES !!!!")
